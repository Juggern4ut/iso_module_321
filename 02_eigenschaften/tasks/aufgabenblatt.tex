...
Ein Ticketsystem für Konzerte hat:

\begin{itemize}
  \item sehr viele Lesezugriffe auf verfügbare Veranstaltungen,
  \item vergleichsweise wenige, aber kritische Schreibvorgänge beim Kauf.
\end{itemize}

\begin{enumerate}
  \item Warum könnte \term{CQRS} hier sinnvoll sein?
  \item Was wäre typischerweise Aufgabe der \textbf{Command Side}?
  \item Was wäre typischerweise Aufgabe der \textbf{Query Side}?
  \item Nennen Sie \textbf{einen Vorteil} und \textbf{einen Nachteil} dieses Musters.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Begründen Sie, ob Sie in diesem Szenario \term{CQRS} oder \term{Event Sourcing} einsetzen würden oder bewusst nicht.
\end{enumerate}
